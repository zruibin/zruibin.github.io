...
for (var index = 0; index < imgList.length; index++) {
    ...
    if (element.src.substring(0, 10) == "data:image") continue;
    ...
    element.addEventListener('click', function (e) {
      ...
      openPreview()
      previewListenerEventAction(true);
    })
}
    ...
    setTimeout(() => {
      if (isMove) {
        ...
        setTimeout(() => {
          ...
          previewListenerEventAction(false);
        }, 300)
      }
    }, 280)
  }
...
function previewPointercancel(e) {
  touches.clear() // 可能存在特定事件导致中断，真机操作时 pointerup 在某些边界情况下不会生效，所以需要清空
}

function previewListenerEventAction(show) {
  if (!isMobile()) {
    return;
  }

  if (show) {
    window.addEventListener('pointerdown', previewPointerdown);
    window.addEventListener('pointerup', previewPointerup);
    window.addEventListener('pointermove', previewPointermove);
    window.addEventListener('pointercancel', previewPointercancel);
  } else {
    window.removeEventListener('pointerdown', previewPointerdown);
    window.removeEventListener('pointerup', previewPointerup);
    window.removeEventListener('pointermove', previewPointermove);
    window.removeEventListener('pointercancel', previewPointercancel);
  }
}
